# References

## The method

Aina, A.; Hsueh, S. C. C.; Plotkin, S. S. PROTHON: A Local Order
Parameter-Based Method for Efficient Comparison of Protein Ensembles.
*Journal of Chemical Information and Modeling* **2023**, *63* (11), 3453–3461.
[doi:10.1021/acs.jcim.3c00145](https://doi.org/10.1021/acs.jcim.3c00145)

## Structural measures

McGibbon, R. T.; Beauchamp, K. A.; Harrigan, M. P.; Klein, C.; Swails, J. M.;
Hernández, C. X.; Schwantes, C. R.; Wang, L.-P.; Lane, T. J.; Pande, V. S.
MDTraj: A Modern Open Library for the Analysis of Molecular Dynamics
Trajectories. *Biophysical Journal* **2015**, *109* (8), 1528–1532.
[doi:10.1016/j.bpj.2015.08.015](https://doi.org/10.1016/j.bpj.2015.08.015)

Shrake, A.; Rupley, J. A. Environment and Exposure to Solvent of Protein Atoms.
Lysozyme and Insulin. *Journal of Molecular Biology* **1973**, *79* (2),
351–371.
[doi:10.1016/0022-2836(73)90011-9](https://doi.org/10.1016/0022-2836(73)90011-9)

## Distances between distributions

Lin, J. Divergence Measures Based on the Shannon Entropy. *IEEE Transactions on
Information Theory* **1991**, *37* (1), 145–151.
[doi:10.1109/18.61115](https://doi.org/10.1109/18.61115)

Endres, D. M.; Schindelin, J. E. A New Metric for Probability Distributions.
*IEEE Transactions on Information Theory* **2003**, *49* (7), 1858–1860.
[doi:10.1109/TIT.2003.813506](https://doi.org/10.1109/TIT.2003.813506)

Villani, C. *Optimal Transport: Old and New*; Grundlehren der mathematischen
Wissenschaften 338; Springer: Berlin, 2009.
[doi:10.1007/978-3-540-71050-9](https://doi.org/10.1007/978-3-540-71050-9)

Delon, J.; Salomon, J.; Sobolevski, A. Fast Transport Optimization for Monge
Costs on the Circle. *SIAM Journal on Applied Mathematics* **2010**, *70* (7),
2239–2258.
[doi:10.1137/090772708](https://doi.org/10.1137/090772708)

Kuiper, N. H. Tests Concerning Random Points on a Circle. *Proceedings of the
Koninklijke Nederlandse Akademie van Wetenschappen, Series A* **1960**, *63*,
38–47.
[doi:10.1016/S1385-7258(60)50006-0](https://doi.org/10.1016/S1385-7258(60)50006-0)

## Density estimation

Silverman, B. W. *Density Estimation for Statistics and Data Analysis*;
Monographs on Statistics and Applied Probability; Chapman and Hall: London,
1986.
[doi:10.1201/9781315140919](https://doi.org/10.1201/9781315140919)

Taylor, C. C. Automatic Bandwidth Selection for Circular Density Estimation.
*Computational Statistics & Data Analysis* **2008**, *52* (7), 3493–3500.
[doi:10.1016/j.csda.2007.11.003](https://doi.org/10.1016/j.csda.2007.11.003)

## Statistical testing

Good, P. I. *Permutation, Parametric, and Bootstrap Tests of Hypotheses*, 3rd
ed.; Springer Series in Statistics; Springer: New York, 2005.
[doi:10.1007/b138696](https://doi.org/10.1007/b138696)

Benjamini, Y.; Hochberg, Y. Controlling the False Discovery Rate: A Practical
and Powerful Approach to Multiple Testing. *Journal of the Royal Statistical
Society, Series B* **1995**, *57* (1), 289–300.
[doi:10.1111/j.2517-6161.1995.tb02031.x](https://doi.org/10.1111/j.2517-6161.1995.tb02031.x)

Kish, L. *Survey Sampling*; Wiley: New York, 1965.

## Comparing whole distributions

Gretton, A.; Borgwardt, K. M.; Rasch, M. J.; Schölkopf, B.; Smola, A. A Kernel
Two-Sample Test. *Journal of Machine Learning Research* **2012**, *13*,
723–773.
[jmlr.org/papers/v13/gretton12a.html](https://www.jmlr.org/papers/v13/gretton12a.html)

Lopez-Paz, D.; Oquab, M. Revisiting Classifier Two-Sample Tests. *International
Conference on Learning Representations*, 2017.
[arXiv:1610.06545](https://arxiv.org/abs/1610.06545)

## Coverage and fidelity

Sajjadi, M. S. M.; Bachem, O.; Lucic, M.; Bousquet, O.; Gelly, S. Assessing
Generative Models via Precision and Recall. *Advances in Neural Information
Processing Systems* **2018**, *31*.
[arXiv:1806.00035](https://arxiv.org/abs/1806.00035)

Kynkäänniemi, T.; Karras, T.; Laine, S.; Lehtinen, J.; Aila, T. Improved
Precision and Recall Metric for Assessing Generative Models. *Advances in
Neural Information Processing Systems* **2019**, *32*.
[arXiv:1904.06991](https://arxiv.org/abs/1904.06991)

## Sequence alignment

Needleman, S. B.; Wunsch, C. D. A General Method Applicable to the Search for
Similarities in the Amino Acid Sequence of Two Proteins. *Journal of Molecular
Biology* **1970**, *48* (3), 443–453.
[doi:10.1016/0022-2836(70)90057-4](https://doi.org/10.1016/0022-2836(70)90057-4)

Gotoh, O. An Improved Algorithm for Matching Biological Sequences. *Journal of
Molecular Biology* **1982**, *162* (3), 705–708.
[doi:10.1016/0022-2836(82)90398-9](https://doi.org/10.1016/0022-2836(82)90398-9)

Henikoff, S.; Henikoff, J. G. Amino Acid Substitution Matrices from Protein
Blocks. *Proceedings of the National Academy of Sciences* **1992**, *89* (22),
10915–10919.
[doi:10.1073/pnas.89.22.10915](https://doi.org/10.1073/pnas.89.22.10915)
